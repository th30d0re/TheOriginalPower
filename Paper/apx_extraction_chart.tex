...
This convention is a modeling choice. It is adopted because it makes the reflected wave $b$ the
formal object corresponding to organized resistance, which is the quantity the historical chapters
track.
...
$W = \psi_m + j\psi_s$ specifies a position on the chart.
...
with a period the carrier fixes.
...
a negative resistance grows without bound at the frequency where the loop gain exceeds unity.
...
natural frequencies. The Bode--Fano criterion bounds the match a reactive load admits across a
finite bandwidth. For a load reducible to a parallel $RC$ combination,
...
Chapter~\ref{ch:spectral_carrier} reports exploratory dominant spectral periods recovered by
Lomb--Scargle from the selected SCOTUS corpus: race at $3.6$~yr and gender at $6.2$~yr. The
sexuality period is unresolved. Modeling each axis as a series resonant branch driven at the
four-year carrier, the normalized branch impedance is
...
The race and gender detuning values follow from those exploratory period estimates. The sexuality
row remains unresolved:

\begin{center}
\begin{tabular}{lccc}
\toprule
Axis & Dominant period & $\omega/\omega_{0,k}$ & $\delta_k$ \\
\midrule
Race       & $3.6$~yr & $0.900$ & $-0.211$ \\
Gender     & $6.2$~yr & $1.550$ & $+0.905$ \\
Sexuality  & threshold-activated & --- & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tikzpicture}[scale=1.0]
  ...
  % --- exploratory axis placements (Q_k = 3 illustrative) ---
  ...
  % Gender: z = 0.50 + j1.357  ->  Gamma = (0.267, 0.663)
  \fill[orange!85!black] (1.068,2.652) circle (3pt);
  \node[font=\scriptsize\bfseries, orange!85!black, right=3pt] at (1.068,2.652) {Gender};
...
point after $\lambda/4$ (Theorem~\ref{thm:quarter-wave}). Coloured markers place the race and gender
sub-bands using exploratory dominant periods recovered by Lomb--Scargle from the selected SCOTUS
corpus. Sexuality has no resolved period and is unplotted. Equation~\ref{eq:xc.14-branch-impedance}
sets the reactive component using an illustrative $Q_k = 3$, which carries Tier~3 status.}
...
Three quantities in this appendix carry assumed values, and each is resolvable with existing data.

$Z_0$ is defined structurally in Section~\ref{sec:xc_normalization} and awaits a fitted value.
Estimating it from the congressional-record spectral series would place the per-axis impedances on
an absolute scale and upgrade the placements in Figure~\ref{fig:extraction_chart} from Tier~3 to
Tier~2.
