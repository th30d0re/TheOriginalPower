...
\mathfrak{T}_{\alpha \to \beta} \;:\; \mathcal{C}_\alpha \;\longrightarrow\; \mathcal{C}_\beta,
...
when the correspondence preserves the relations the claim asserts. The symbol $\mathfrak{T}$ is
...
$\mathfrak{T}_{\alpha \to \beta}(c)$ on every domain $D_\beta$ inside the principle's scope. A
...
\mathfrak{T}_{\alpha \to \beta}(c) \text{ holds on } D_\beta
...
hold on $D_\alpha$. If $\mathfrak{T}_{\alpha \to \beta}(c)$ fails on some $D_\beta$ with
...
pre-images under $\mathfrak{T}^{-1}$ match claims the historical chapters had already established
...
\begin{enumerate}
    ...
    whose pre-image under $\mathfrak{T}^{-1}$ contradicts an established result of the historical
...
$\mathfrak{T}_{\beta \to \gamma} \circ \mathfrak{T}_{\alpha \to \beta} =
\mathfrak{T}_{\alpha \to \gamma}$ holds in general determines whether chains of transports
