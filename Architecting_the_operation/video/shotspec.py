"""Turn a shot list into machine-readable specs, and check its provenance.

The shot lists are the authored artifact: prose, written for a human editor.
This reads one and emits a JSON spec per shot, so the same shot can be sent to
any image model and the comparison between them is about the model rather than
about how the prompt was phrased that day.

It also enforces the provenance key the lists declare. A `[book]` shot has to
cite a file and line that exists; a `[data]` shot has to name a real file under
Paper/data/; a shot with no tag at all gets flagged. That key is only worth
having if something checks it, and 145 shots is past the point of checking by
hand.

    python shotspec.py ATO_EP03_shotlist.md --out specs/ATO_EP03.json \
        --manifest ../../outputs/ATO_EP03_five_levels/episode_manifest.json

With a manifest the anchors resolve to real milliseconds, which is the
alignment procedure the lists describe, done once instead of per shot.
"""
...
def spec_for(shot: Shot, names: dict | None = None, turns: dict | None = None) -> dict:
...
def build(shotlist: Path, manifest: Path | None, script: Path | None = None) -> dict:
    names, turns = (None, None)
    if manifest:
        names, turns = _manifest_lookup(manifest, script)
    shots = parse(shotlist)
    specs = [spec_for(shot, names, turns) for shot in shots]
    return {
