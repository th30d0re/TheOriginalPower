...
\newcommand{\RR}{\textit{The Mathematics of Oppression}}
...
\subsection{Patriarchy, Misogyny, and Misandry: The Extraction Architecture and Its Cognitive Payloads}

A common diagnostic error treats patriarchy as synonymous with male power. This conflates the extraction architecture with the population that happens to occupy its Enforcement and Buffer tiers. The framework developed in \RR{} distinguishes three layers on the gendered axis:

\begin{enumerate}
  \item \textbf{Patriarchy} is the extraction architecture---the overarching system that optimizes Elite extraction through asymmetric autonomy restriction on the gendered axis.
  \item \textbf{Misogyny} is the cognitive payload, the exhaust prejudice that hijacks Bayesian priors to program the prior that female subjugation and reproductive extraction are natural.
  \item \textbf{Misandry} is the dual cognitive payload, the exhaust prejudice that programs the prior that male disposability, kinetic obligation, and provider sacrifice are natural.
\end{enumerate}

The popular formula ``prejudice + power = oppression'' conflates the system with the exhaust it generates. When analysts say that misandry ``cannot exist'' because men hold power, they are accepting the exact prior the system installed: they are mapping the gendered axis as a flat binary in which all men occupy the In-group and all women occupy the Out-group. The five-tier architecture described in \RR{} is more precise: the Elite extract; the Puppet Class translate; the Enforcement Class actuates; the Buffer Class absorbs friction; and the Out-group bears the compounding burden. Everyday men are not the Elite. They are the Enforcement and Buffer tiers---disposable utilities who receive a status wage (``provider,'' ``protector'') in exchange for absorbing kinetic risk.

Misogyny programs the prior that women's subjugation is natural, making reproductive and domestic extraction seem like affection or tradition. Misandry programs the prior that men's disposability is natural, making hazardous labor, carceral filling, and military conscription seem like duty or honor. Both prejudices are real; both serve the same kernel. Denying misandry because some men are In-group on a flat map is not dismantling power. It is helping the interference engine keep running.

\subsubsection{The Material Outputs of Male Disposability}

The claim that patriarchy ``empowers men'' must be tested against the material data. If everyday men were the beneficiaries of the architecture, their bodies would not be its primary extraction surface:

\begin{itemize}
  \item \textbf{Occupational extraction.} Bureau of Labor Statistics data show men account for over 92\% of fatal occupational injuries. The system routes hazardous labor through male bodies and normalizes the routing through the narrative of male toughness.\SourceNote{BLS Census of Fatal Occupational Injuries.}
  \item \textbf{Carceral extraction.} US Sentencing Commission data show men receive federal sentences roughly 30\% longer than women for identical crimes. The state uses male bodies to fill carceral capacity, then narrates the disparity as ``men are more dangerous'' rather than ``men are more disposable.''\SourceNote{US Sentencing Commission, demographic reporting on federal sentences.}
  \item \textbf{Custody architecture.} Family courts default to maternal custody not because they ``favor women,'' but because the gendered partition still treats women as the primary reproductive infrastructure. Fathers face immense friction because the child support apparatus needs a designated financial payer, and the custody machinery is calibrated to the nuclear family---a unit optimized for extraction. The mother receives custody; the father receives an extraction order disguised as an obligation.
\end{itemize}

None of these data invalidate feminist critique of male violence, male entitlement, or patriarchal subordination of women. They simply show that the architecture does not empower everyday men. It extracts from them through disposability, just as it extracts from women through subjugation. The extraction modes are asymmetric---disposability is not the same as subordination---but they are co-produced by the same kernel.

\subsubsection{Delegated Privilege and the Flat-Map Error}

The flat-map error appears most clearly in historical cases where women held In-group authority on one axis while remaining Out-group on another. During the Jim Crow era, white women held local In-group authority over Black men on the racial axis---they could accuse, surveil, and mobilize lethal violence. But they remained Out-group on the gender axis relative to the white male Elite. Their power was not apex control. It was a \textit{delegated enforcement privilege} maintained by the Elite to police the racial partition without risking Elite bodies.

This matters because intersection is not additive. A gay Black man and a straight Black woman do not occupy the same structural position, not because one is ``more oppressed'' in some abstract sense, but because the system calibrates extraction by intersection. The gay Black man faces compounded extraction on sexuality and race; the straight Black woman faces compounded extraction on gender and race. The comparison is not a competition. It is a calibration map.

\subsubsection{The Invariant}

The system's only invariant is that the Elite's share of extraction never declines. In the formal terms of \RR:

\begin{equation}
  \Delta_{\max} = 0,
\end{equation}

where $\Delta_{\max}$ represents the maximum allowable decline in Elite extraction rate. Everything else---including the gender war itself---is \textit{min-management}: the tuning of grievance amplitude so that cross-sex coherence remains below the threshold at which class solidarity could form. Misogyny and misandry are the cognitive fuel for this tuning. They ensure that each sex experiences the other as the proximate enemy, while the extraction kernel remains invisible.
...
\endhead
Transcript files & 56 \\
Files with retrievable Instagram metadata & 55 \\
Unavailable media-info endpoint results & 1 \\
Total public plays across retrievable reels & 24,297,680 \\
Median public plays among retrievable reels & 159,066 \\
Lowest retrievable public play count & 2,066 \\
Highest retrievable public play count & 6,498,495 \\
Total likes across retrievable reels & 1,544,875 \\
Total comments across retrievable reels & 88,321 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\texttt{DXSLGz0jV-k} & @savannafaithstone & 6,498,495 & 376,141 & 10,904 \\
\texttt{DXRiZuYiOh5} & @jestressjoan & 1,827,143 & 125,024 & 6,796 \\
\texttt{DVIFo7hEbzf} & @amirzenofficial & 1,731,627 & 93,041 & 2,796 \\
\texttt{DV7HsBkEf3x} & @nardosedaily & 1,310,597 & 78,157 & 14,968 \\
\texttt{DSQtiyREsFf} & @sophiaesperanza & 960,507 & 47,697 & 2,931 \\
\bottomrule
\end{longtable}
...
\subsection{Runtime Evidence for the Extraction Architecture}

Reading the corpus through the frame developed in Section~2.3 produces additional insight that the descriptive pattern tables do not capture on their own. The frame treats patriarchy as an extraction architecture, misogyny and misandry as its dual cognitive payloads, and the gender war as \textit{min-management} calibrated to keep cross-sex coherence below the solidarity threshold. The reels confirm each layer.

\subsubsection{Patriarchy as Extraction Architecture, Not Male Empowerment}

The corpus contains multiple reels in which men are framed not as empowered elites but as disposable utilities whose bodies and labor are routed through hazardous, carceral, and custodial channels.

\begin{itemize}
  \item \textbf{Kinetic infrastructure} (@jestressjoan, \texttt{DXRiZuYiOh5}): The speaker catalogs male bodily extraction directly---``We need our men off the streets. We need the men out of the wars. We need the men to stop breaking their bodies.'' The framing is maintenance-oriented, not empowerment-oriented: men are depreciating assets whose utility (strength, protection, discipline) must be preserved so they remain available for collective use. The line ``We need them to be loved. We need them to be cared for'' is striking because it appears \textit{after} the inventory of extraction, as if love is the lubricant that keeps the machinery running.
  
  \item \textbf{Custody extraction} (@nardosedaily, \texttt{DV7HsBkEf3x}): A first-person testimony describes how the speaker's father was extracted from her life not through abandonment but through maternal gatekeeping. The mother, acting as a delegated enforcer of the family-system boundary, weaponized the child against the father. The child then internalized the extraction as paternal rejection, producing lifelong attachment damage. The father is not absent because he wields patriarchal power; he is absent because he was removed from the household and his relationship converted into a wound.
  
  \item \textbf{Platform extraction} (@yourstrulypalakk, \texttt{DU2D54MAaH\_}): The speaker names the business model explicitly: ``The male loneliness epidemic is not just a social issue, it is a business model. Social media companies figured out that the angry lonely men, they click more, they buy more and they stay longer.'' Men are not patriarchs in this frame; they are \textit{profitable surplus}---resource nodes whose rage and aspirational hustle are harvested by algorithms. Simultaneously, women are fed the flat-map error (``men are so bad and they're evil''), collapsing the distinction between elite power-holders and everyday men so that the harvest can continue.
  
  \item \textbf{Carceral asymmetry} (@theisabelbrown, \texttt{DWRtSEHABsI}): The reel documents a joke-violence double standard in which female-perpetrated simulated intimate violence is circulated as entertainment while male-perpetrated simulated violence would trigger immediate imprisonment. The extraction mechanism is the state's readiness to fill carceral capacity with male bodies while treating women's violence as algorithmically legible comedy. Men are treated as bodies that can absorb violence without institutional protection.
\end{itemize}

\subsubsection{Cognitive Payloads in the Wild}

The corpus also confirms that misogyny and misandry operate not as random prejudice but as \textit{exhaust prejudices}---cognitive payloads that program Bayesian priors to make extraction seem natural.

\begin{itemize}
  \item \textbf{Misogyny payload} (@iamqueenlilah, \texttt{DXnN\_uWgHQ2}): The reel stages a debate in which the male-position voice programs female subordination as biological destiny: ``women are naturally designed to follow and men are designed to lead,'' ``she was placed by his side to protect his heart,'' and ``Women are too emotional.'' The prior being installed is that hierarchy is theological/biological common sense, not a constructed power arrangement that extracts reproductive and domestic labor.
  
  \item \textbf{Misandry payload---complicity continuum} (@valeriajoffre, \texttt{DTjSC0EjF2Z}): The speaker replaces ``not all men'' with a spectrum in which male dangerousness is the default and innocence must be actively proven. The continuum runs from ``direct violators'' to ``enablers'' to ``bystanders,'' with only a narrow ``green zone'' of men who actively police other men granted provisional innocence. The prior being installed is that male moral suspicion is the natural state; male safety must be earned through continuous performative labor.
  
  \item \textbf{Misandry payload---provider sacrifice} (@millionaire.mindsetspod, \texttt{DXcd0jiCIOA}): The reel naturalizes male disposability through provision: ``A man with a family he loses that freedom. Now every decision he makes it runs through a different filter. It's no longer what do I want but what happens to them if I don't? And that's the sacrifice.'' The prior being installed is that a man's value is measured by what he absorbs silently so others do not have to. His wants, health, and aspirations are automatically secondary once he has dependents.
  
  \item \textbf{Misandry payload---violent enforcement} (@streetlighteyesdontdie, \texttt{DSOJzQPEfWX}): The speaker programs male worth as conditional on willingness to use violence and accept legal risk: ``You have to speak up and make it known that you fucking hate sexual assailants, and you have to make them more afraid of you.'' The prior being installed is that a ``good man'' is not merely non-harmful but an active violent enforcer whose legal safety is acceptable collateral.
\end{itemize}

A notable finding is the \textit{absence} of dual-payload reels in this sample. No single reel simultaneously programs both female subjugation and male disposability as natural priors. The gender-war content tends to \textit{polarize} into one payload or the other, or to resist payloads entirely (as with @jestressjoan and @sophiaesperanza). This polarization is itself evidence of the interference engine: the system does not need both prejudices in the same speaker; it only needs each sex to encounter the prejudice that justifies its own extraction while blaming the other sex for the result.

\subsubsection{Min-Management: The Platform as Interference Tuner}

The corpus provides runtime evidence that the platform functions as a min-management system. Repair-oriented reels that attempt to restore cross-sex coherence collectively reach approximately one million public plays. Grievance-amplification reels that raise $G_m$ or $G_w$ while suppressing $C_{m,w}$ collectively reach approximately eleven million public plays---a rough $11{:}1$ ratio.

This is not proof of platform conspiracy, but it is consistent with the min-management prediction: the distribution system rewards content that makes the viewer feel accused, vindicated, endangered, or superior, while content that invites mutual diagnosis receives markedly less circulation.

The mechanisms are visible in the transcripts:

\begin{itemize}
  \item \textbf{Evidence weaponization} (@savannafaithstone, \texttt{DXSLGz0jV-k}): Every statistic---divorce rates, same-sex DV rates, mental-health differentials---is deployed as a round in a sex-class war, not as a shared diagnostic. The speaker does not invite collaborative investigation; she uses numbers to condemn. The claim audit notes that the 75--80 percent divorce-initiation figure is higher than the commonly cited Rosenfeld/ASA 69 percent, and the same-sex DV ranking is oversimplified. The numbers travel not because they are precise but because they are ammunition.
  
  \item \textbf{Structural stress redirected into sex-class accusation} (@liam.out.loud, \texttt{DXXOjqgksYE}): The speaker converts structural phenomena (institutional exposure differentials, credential inflation, labor-market polarization) into ontological accusation: young women ``choose'' to see men as enemies, and institutions were ``feminized'' and therefore corrupted. The actual class mechanisms (who funds the lawsuits, who captures the surplus from cheaper educated labor) remain invisible; women and feminized institutions are the proximate enemy.
  
  \item \textbf{The repair minority} (@yourstrulypalakk, \texttt{DU2D54MAaH\_}; @asknivea, \texttt{DRCugr6k1Mc}): Both speakers explicitly reject the proximate-enemy frame. @yourstrulypalakk names the extraction kernel (``people in power,'' platform monetization) and refuses to let either sex be the enemy. @asknivea calls for accountability, space, and connection: ``Stop playing team woman versus team men. Start making it you versus your pattern.'' These are minority currents---lower circulation, lower algorithmic reward---but they prove that cross-sex coherence is still linguistically available.
\end{itemize}

\subsubsection{Delegated Enforcement and the Flat-Map Error}

The corpus contains multiple instances of the flat-map error: the conflation of everyday men with the Elite, and the treatment of women as unified Out-group.

\begin{itemize}
  \item \textbf{Flat-map from the male side} (@liam.out.loud, \texttt{DXXOjqgksYE}): The ``daycare governance model'' sorts all men into ``predators to be removed'' by default, with no gray zone. This is the flat-map error in institutional form: the system defines men as threats by default, authorizing their extraction without individualized process.
  
  \item \textbf{Flat-map from the female side} (@valeriajoffre, \texttt{DTjSC0EjF2Z}): The complicity continuum makes male moral suspicion the default state. The flat-map is not total---there is a ``green zone''---but the green zone is narrow and conditional, requiring continuous performative labor to maintain.
  
  \item \textbf{Flat-map diagnosis from the outside} (@linusoyugi, \texttt{DYX232eMWHP}): The Kenyan creator performs an external diagnosis: ``Feminism teaches women that they are the oppressed and men are the oppressors. The manosphere teaches men that they are the oppressed and women are the oppressors.'' He identifies that both movements collapse the class/structural distinction between elite men and working-class men, producing the same flat-map from opposite poles.
  
  \item \textbf{Delegated enforcement} (@nardosedaily, \texttt{DV7HsBkEf3x}): The mother acts as a delegated enforcer of the custody boundary, weaponizing the child against the father. Her power is not apex control; it is a gatekeeping privilege maintained by the family-law system, which needs a designated custodian and a designated payer. She enforces the extraction; she does not design it.
\end{itemize}

\subsubsection{Global Travel and Calibration}

One finding that strengthens the architecture claim is the global travel of the gender-war narrative. The newest reel in the corpus (@linusoyugi, \texttt{DYX232eMWHP}) is from a Kenyan creator who rehearses the same script as US creators: feminism versus manosphere, de-centering, resentment, and the collapse of the family. The structural conditions differ---Kenya has different carceral, welfare, and labor regimes---but the narrative pattern is nearly identical. This suggests the gender war is not locally determined but \textit{platform-distributed}: the algorithm transmits the interference pattern across national boundaries, calibrating it to local grievance while preserving the same extraction kernel.
...
\section{The Mathematics of Oppression Layer: Why the War Is Structurally Useful}

From the perspective of \RR, the gender war is useful because it converts potential class coherence into sex-class grievance. Men and women share exposure to debt, wage stagnation, housing unaffordability, surveillance, healthcare precarity, family instability, and algorithmic behavioral manipulation. But the gendered interface encourages each sex to experience the other as the proximate cause of its deprivation.
...
In the terms of \RR, the gendered axis becomes a phase-loading system:
...
\begin{itemize}
  \item \path{ModernMisandry/pseudo_hybrid_asymmetric_monogamy_analysis/Pseudo_Hybrid_Asymmetric_Monogamy.md}
  \item \path{Paper/Redefining_Racism.tex}
\end{itemize}
